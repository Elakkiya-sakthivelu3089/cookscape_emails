...
export async function seedDatabase() {
  ...
  const defaultPassword = await bcrypt.hash('Cookscape@123', 10);
  ...
  for (const ch of channels) {
    const existing = await prisma.chatRoom.findFirst({ where: { name: ch.name, type: 'CHANNEL' } });
    if (!existing) {
  ...
  for (const t of templates) {
    const existing = await prisma.emailTemplate.findFirst({ where: { title: t.title } });
    if (!existing) {
