...
class MailController {
    ...
    static async testSmtpConnection(_req, res) {
        try {
    ...
    static async handleInboundWebhook(req, res) {
        try {
            const body = req.body || {};
            ...
            const email = await mailEngine_js_1.MailEngine.receiveInboundEmail({
                senderEmail,
                senderName,
                recipientEmail,
                subject,
    ...
    static async simulateInboundEmail(req, res) {
        try {
            const { from = 'client.ananya@gmail.com', fromName = 'Ananya Client (Villa 402)', to = 'priya.designer@cookscape.com', subject = 'Update on Living Room Floor Plan & Colors', html = '<p>Hi Priya,<br/><br/>We loved the latest 3D render of the living room! Could we explore warm oak veneer for the TV console?<br/><br/>Best regards,<br/>Ananya</p>', text = 'Hi Priya,\n\nWe loved the latest 3D render of the living room! Could we explore warm oak veneer for the TV console?\n\nBest regards,\nAnanya', threadId, } = req.body;
            const email = await mailEngine_js_1.MailEngine.receiveInboundEmail({
                senderEmail: from,
                senderName: fromName,
                recipientEmail: to,
