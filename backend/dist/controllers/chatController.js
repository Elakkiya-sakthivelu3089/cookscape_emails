...
class ChatController {
    ...
    static async getUserRooms(req, res) {
        try {
            ...
            const rooms = await index_js_1.prisma.chatRoom.findMany({
                where: {
                    isArchived: false,
                    OR: [
                        { type: 'CHANNEL' },
    ...
    static async getRoomMessages(req, res) {
        try {
            ...
            const room = await index_js_1.prisma.chatRoom.findUnique({
                where: { id: roomId },
                include: {
                    members: {
                        include: {
            ...
            const messages = await index_js_1.prisma.chatMessage.findMany({
                where: { roomId },
            ...
            await index_js_1.prisma.chatMember.upsert({
                where: {
                    roomId_userId: {
                        roomId,
                        userId: req.user.id,
    ...
    static async sendMessage(req, res) {
        try {
            ...
            // Create message
            const message = await index_js_1.prisma.chatMessage.create({
                data: {
                    roomId,
            ...
            await index_js_1.prisma.chatRoom.update({
                where: { id: roomId },
                data: { updatedAt: new Date() },
            });
            // Emit real-time message to socket room
            (0, socketService_js_1.emitChatMessage)(roomId, message);
    ...
    static async createRoom(req, res) {
        try {
            ...
            if (type === 'DIRECT') {
                ...
                const existingRoom = await index_js_1.prisma.chatRoom.findFirst({
                    where: {
                        type: 'DIRECT',
                        AND: [
            ...
            (0, socketService_js_1.emitRoomUpdate)(allMembers, room);
            res.status(201).json({
